...
void FlightControl_UpdateAttitude(MPU6050_Data_t *mpuData)
{
    ...
    mpuData->yaw  += mpuData->gyro_z / GYRO_SCALE * 0.098f; // 偏航角（单位：度）
    // 保持角度在 -180° 到 +180° 范围内
...
void FlightControl_CalculateOutput(void)
{
    ...
    // 计算PID输出
    roll_output = PID_Compute(&fc_pid.pid_roll, fc_attitude.target.roll, mpu6050_data.roll);
    pitch_output = PID_Compute(&fc_pid.pid_pitch, fc_attitude.target.pitch, mpu6050_data.pitch);
    yaw_output = PID_Compute(&fc_pid.pid_yaw, fc_attitude.target.yaw, mpu6050_data.yaw);
    
    // 计算每个电机的输出
    // M1: 右前 (+pitch, -roll, -yaw)
    motor1_output = (uint)(fc_control.throttle + pitch_output - roll_output - yaw_output);
    
    // M2: 左前 (+pitch, +roll, +yaw)
    motor2_output = (uint)(fc_control.throttle + pitch_output + roll_output + yaw_output);
    
    // M3: 左后 (-pitch, +roll, -yaw)
    motor3_output = (uint)(fc_control.throttle - pitch_output + roll_output - yaw_output);
    
    // M4: 右后 (-pitch, -roll, +yaw)
    motor4_output = (uint)(fc_control.throttle - pitch_output - roll_output + yaw_output);
